...
(defun handleExp (expr dvar)
  (cond
    ...
    ((isAx (caddr expr) dvar)
     (make-div `(^ e ,(caddr expr))  (getA (caddr expr) dvar))
     )
    ...
    ((or (isXbyA (caddr expr) dvar) (isXbyAplusb (caddr expr) dvar))
     (make-prod `(^ e ,(caddr expr))  (getA (caddr expr) dvar))
     )
    (T `(^ e ,expr))
    )
  )
...
(defun handleAExp (expr dvar)
  (cond
    ((eq dvar (caddr expr) )
     `(/ (^ ,(cadr expr) ,dvar) (log ,(cadr expr)))
     )
    ((isAx (caddr expr) dvar)
     `(/ ,(make-div `(^ ,(cadr expr) ,(caddr expr))  (getA (caddr expr) dvar)) (log ,(cadr expr)))
     )
    ((isXb (caddr expr) dvar)
     `(/ (^ ,(cadr expr) ,(caddr expr)) (log ,(cadr expr)))
     )
    ((isAxb (caddr expr) dvar)
     `(/ ,(make-div `(^ ,(cadr expr) ,(caddr expr))  (getA (caddr expr) dvar)) (log ,(cadr expr)))
     )
    ((or (isXbyA (caddr expr) dvar) (isXbyAplusb (caddr expr) dvar))
     `(/ ,(make-prod `(^ ,(cadr expr) ,(caddr expr))  (getA (caddr expr) dvar)) (log ,(cadr expr)))
     )
    (T `(* (^ ,(cadr expr) ,(caddr expr)) (log ,(cadr expr))))
    )
  )
